...
class Cell:
    ...
    def update_cell(self):
        if self.variant != -1:
            self.image = Image.new("RGBA", _Images['cell_dark'].size)
            self.image.paste(_Images[f'{ball_by_index[self.color]}_{self.variant}'], (5, 5))
            if self.dark:
                self.image = Image.alpha_composite(_Images['cell_dark'], self.image)
            else:
                self.image = Image.alpha_composite(_Images['cell_light'], self.image)
...
def load_Images():
    global _Images, Images, ball_by_index
    _Images.update({
        "cell" : Image.open("Images/cell-bgr.png").convert('RGBA'),
        "page" : Image.open("Images/page-bgr.png").convert('RGBA'),
    })
    _Images.update({
        "cell_dark" : _Images["cell"].crop( (2, 1, 67, 66) ),
        "cell_light" : _Images["cell"].crop( (2, 70, 67, 135) ),
    })
    _Images_of_balls = {
    ...
    for k, v in _Images_of_balls.items():
        for i in range(7):
...
def create_info(canvas):
    ...
    canvas.create_text(
        x_pos,
        y_pos,
        text="L.I.N.E.S.", font=("Fixedsys", 17), fill="#FFFFFF",
        anchor=W)
    ...
    canvas.create_text(
        x_pos,
        y_pos,
        text="Score: ", font=("Fixedsys", 17), fill="#FFFFFF",
        anchor=NW)
    ...
    score = canvas.create_text(
        x_pos + 100,
        y_pos,
        text=str(score_val), font=("Fixedsys", 17), fill="#FFFFFF",
        anchor=NW)

    y_pos += 50
    Button(root, text='Сделать ход',
        fg='white', bg='#666666', activebackground='#777777', activeforeground='white',
        command=make_move,font="Fixedsys").place(x=x_pos, y=y_pos)

    y_pos += 50
    canvas.create_text(
        x_pos,
        y_pos,
        text="Подсказка:", font=("Fixedsys", 15), fill="#888888",
        anchor=NW)
    ...
    Button(root, text='Новая игра',
        fg='white', bg='#666666', activebackground='#777777', activeforeground='white',
        command=new_game,font="Fixedsys").place(x=x_pos, y=y_pos)
...
if __name__ == '__main__':
    ...
    try:
        ...
        while True:
            ...
            if game_over:
                if game_over_label == None:
                    game_over_label = Label(root, font=('Fixedsys', 20), text='игра окончена', fg='white', bg='#666666')
